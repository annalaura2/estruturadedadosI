...
#include <string.h>
#include <stdlib.h>
#include <ctype.h> // biblioteca para usar o size_t - usado para pegar tamanhos

typedef struct
{
    char *content; // ponteiro que recebe um bloco de bytes que vai armazenar a string
} String;

String stringCreate(char *); // função que cria a string
int stringDestroy(String *); // função que destroí a string

size_t stringLength(String); // 1 forma de calcular o tamanho da string
size_t stringLength2(String); // 2 forma de calcular o tamanho da string

char stringCharAt(String, unsigned int); // função para retornar o caractere de uma certa posição 
String stringConcat(String, String);
String stringToLower(String); // colocar a string em letras minúsculas 
...
String stringCreate(char *string)
{
    ...
    length = strlen(string); // pegando o tamanho da string
    content = (char *)malloc(length + 1); // alocação
    //lembrete: se tiver que alocar uma string com 10 caracteres, precisa alocar 11

    if (content == NULL)
        return result;

    for (i = 0; i < length; i++) // for que percorre a string até o seu tamanho
    {
        content[i] = string[i];
    }

    content[length] = '\0'; // o último recebe o /0 que o terminador nulo 
